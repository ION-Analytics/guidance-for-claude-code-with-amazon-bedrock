...
// sessionPolicy is an optional IAM JSON policy injected as a session policy (e.g. to restrict
// Bedrock model access to an allowed subset). Pass "" to omit.
func AssumeRoleWithWebIdentity(region, roleARN, idToken string, claims jwt.Claims, maxDuration int, sessionPolicy string) (*AWSCredentials, error) {
