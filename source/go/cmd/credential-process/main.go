...
func (a *credentialApp) run() int {
	...
	if a.cfg.QuotaAPIEndpoint != "" {
		debugPrint("Checking quota before credential issuance...")
		...
		if len(qr.AllowedModels) > 0 {
			...
			if sessionPolicy != "" {
				debugPrint("Injecting model session policy for %d allowed model(s)", len(qr.AllowedModels))
			}
		}
	}
	...
	awsCreds, err := a.getAWSCredentialsWithPolicy(authResult, sessionPolicy)
	if err != nil {
...
func (a *credentialApp) getAWSCredentials(auth *oidc.AuthResult) (*federation.AWSCredentials, error) {
	return a.getAWSCredentialsWithPolicy(auth, "")
}

func (a *credentialApp) getAWSCredentialsWithPolicy(auth *oidc.AuthResult, sessionPolicy string) (*federation.AWSCredentials, error) {
	if a.cfg.FederationType == "direct" {
		debugPrint("Requesting session duration: %ds (%dh)", a.cfg.MaxSessionDuration, a.cfg.MaxSessionDuration/3600)
		if sessionPolicy != "" {
			debugPrint("Injecting model session policy (%d bytes)", len(sessionPolicy))
		}
		return federation.AssumeRoleWithWebIdentity(
			a.cfg.AWSRegion, a.cfg.FederatedRoleARN, auth.IDToken,
			auth.TokenClaims, a.cfg.MaxSessionDuration, sessionPolicy,
		)
	}
...
func (a *credentialApp) quotaTimestampPath() string {
	home, _ := os.UserHomeDir()
	return home + "/.claude-code-session/" + a.profile + "-quota-check.json"
}

func (a *credentialApp) saveQuotaCheckTimestamp() {
	path := a.quotaTimestampPath()
	dir := path[:strings.LastIndex(path, "/")]
	_ = os.MkdirAll(dir, 0700)
	data, _ := json.Marshal(map[string]string{"last_check": time.Now().UTC().Format(time.RFC3339)})
...
func (a *credentialApp) shouldRecheckQuota() bool {
	...
	raw, err := os.ReadFile(a.quotaTimestampPath())
	if err != nil {
		return true
	}
	...
	last, err := time.Parse(time.RFC3339, ts["last_check"])
	if err != nil {
		return true
	}
	interval := a.cfg.QuotaCheckInterval
	if interval == 0 {
		interval = 30
	}
...
// to Bedrock inference profiles NOT in the allowedModels list.
// Uses Resource ARN deny — IAM bedrock:ModelId condition keys are unreliable.
// Returns "" if no restriction is needed or the policy would exceed 2048 bytes.
func buildModelSessionPolicy(allowedModels []string) string {
	allFamilies := map[string]struct{}{
	...
	var deniedARNs []string
	for family := range allFamilies {
		if _, ok := allowed[family]; ok {
			continue
		}
		// Simplify *anthropic.claude-opus* → *opus* for ARN wildcard matching
		simple := strings.TrimPrefix(family, "*anthropic.claude-")
		simple = strings.TrimSuffix(simple, "*")
		deniedARNs = append(deniedARNs, "arn:aws:bedrock:*:*:inference-profile/*"+simple+"*")
	}
	if len(deniedARNs) == 0 {
		return ""
	}

	// Build JSON manually to keep it compact and predictable
	resourcesJSON, _ := json.Marshal(deniedARNs)
	policy := fmt.Sprintf(
		`{"Version":"2012-10-17","Statement":[{"Sid":"AllowAll","Effect":"Allow","Action":"*","Resource":"*"},`+
			`{"Sid":"DenyDisallowedModels","Effect":"Deny","Action":["bedrock:InvokeModel","bedrock:InvokeModelWithResponseStream"],"Resource":%s}]}`,
